...
def iniciar_movimento(event):
    ...
    # Edição de transição tem prioridade, exceto no modo Apagar
    ...
    # --- LÓGICA BASEADA NO MODO (COM CORREÇÃO) ---
    if modo_atual == "arrastar":
        estado_clicado = encontrar_estado_clicado(event)
        
        # Se um estado foi encontrado nas proximidades...
        if estado_clicado:
            # ...verificamos a distância para decidir o que fazer.
            dist = math.dist((event.x, event.y), (estado_clicado.x, estado_clicado.y))
            
            # Se o clique foi DENTRO do raio do estado, prepara para arrastar.
            ...
            # Se o clique foi FORA do raio, cria um novo estado.
            else:
                criar_novo_estado(event.x, event.y)
        # Se nenhum estado foi encontrado (tela vazia), cria um novo estado.
        else:
            criar_novo_estado(event.x, event.y)
            
    elif modo_atual == "selecao":
        # Limpa seleção anterior se clicar no fundo
        for item in itens_selecionados: item.desselecionar()
        itens_selecionados.clear()
        # Começa a desenhar a caixa de seleção
...
def arrastar_objeto(event):
    # Arrastar um único estado (só no modo arrastar)
    if modo_atual == "arrastar" and objeto_arrastado.get("id"):
    ...
    # Desenhar a caixa de seleção (só no modo seleção)
...
icones = {}
try:
    nomes_icones = ["estado", "transicao", "selecionar", "apagar", "salvar"] # <-- ATUALIZE A ORDEM AQUI
