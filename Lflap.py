...
class Transicao:
    # --- MODIFICADO: Adicionado simbolo_pop e string_push ---
    def __init__(self, origem, destino, canvas, simbolos_entrada="ε", simbolo_saida="", 
        ...
        self.simbolo_pop = simbolo_pop    # <-- NOVO
        self.string_push = string_push  # <-- NOVO
    ...
    # --- MODIFICADO: Lógica para exibir o rótulo do AP ---
    def _get_rotulo_texto(self):
        ...
        if tipo_automato_atual == "AP":
            ...
            # O formato padrão é: entrada, pop ; push
            return f"{texto_entrada}, {pop_val} ; {push_val}"
        # --- FIM DA NOVA LÓGICA ---
            
        return texto_entrada # Padrão para AFD, AFN, AFNe, Moore
    ...
    # --- MODIFICADO: Agora 'parseia' o rótulo com base no tipo de autômato ---
    def atualizar_simbolo(self, novo_rotulo_completo):
        ...
        if tipo_automato_atual == "AP":
            try:
                ...
                self.simbolos_entrada = [simbolo_entrada] # APs (deste tipo) leem 1 símbolo por vez
                self.simbolo_pop = partes_virgula[1].strip() if len(partes_virgula) > 1 else "ε"
                
                # Normaliza para 'ε' se estiver vazio
        ...
        # --- LÓGICA EXISTENTE PARA AFs e MOORE ---
        else:
            partes = novo_rotulo_completo.split('/', 1) # Ignora saída
            simbolos_entrada_str = partes[0].strip()
            self.simbolo_saida = "" # Garante que não há saída
            if not simbolos_entrada_str or simbolos_entrada_str.lower() == "ε":
                self.simbolos_entrada = ["ε"]
            else:
                self.simbolos_entrada = [s.strip() for s in simbolos_entrada_str.split(',')]
...
def gerenciar_clique_transicao(event):
    ...
    if transicao_info["origem"] is None:
        ...
        # --- MODIFICADO: Cria a transição base (com pop/push 'ε' por padrão) ---
...
# --- MODIFICADO: Adiciona a lógica de edição do AP ---
def editar_rotulo_transicao(id_item_clicado):
    ...
    if tipo_automato_atual == "AP":
        ...
        if novo_rotulo_completo is not None:
            # A função 'atualizar_simbolo' agora faz o trabalho pesado
            transicao_alvo.atualizar_simbolo(novo_rotulo_completo)

    # --- Lógica existente para AFD, AFN, AFNe, Moore ---
...
def toggle_aceitacao_event(event):
    # --- MODIFICADO: AP também pode ter estados de aceitação ---
...
def mostrar_menu_contexto(event):
    ...
    if acao_em_grupo:
        # --- MODIFICADO: Permite alternar aceitação para APs em grupo ---
        ...
        # --- MODIFICADO: Permite alternar aceitação para AP ---
        if tipo_automato_atual not in ["Mealy", "Moore"]:
...
def renomear_estado(estado):
    ...
    if novo_nome and novo_nome not in estados:
        ...
        # --- MODIFICADO: Atualiza o texto (importante se for Moore) ---
        estado.nome = novo_nome
        estado.atualizar_texto() # Garante que o texto q0/s0 também mude
...
def toggle_aceitacao_selecao():
    ...
    # --- MODIFICADO: Só permite se não for Mealy/Moore ---
...
def animar_token(token_id, origem, destino, callback_ao_finalizar, passo_atual=0, total_passos=30):
    # (Esta função permanece inalterada, mas não será usada pelo simulador de AP)
...
def calcular_fecho_epsilon(estados_origem):
    ...
    while pilha:
        ...
        for t in transicoes:
            # --- MODIFICADO: Apenas AFNe usa esta função para transições épsilon ---
            ...
            # (O AP tem sua própria lógica de épsilon dentro do simular_AP)
    return fecho
...
# --- MODIFICADO: Adiciona o 'elif' para o AP ---
def simular_palavra():
    global tipo_automato_atual
    ...
    # --- NOVO BLOCO ---
        ...
        # APs não têm animação passo-a-passo por token (ainda)
        # Eles usam um simulador diferente devido ao não-determinismo e à pilha
...
# --- NOVO: Motor de Simulação para Autômato com Pilha ---
def simular_AP(palavra, estado_inicial):
    """
    Executa a simulação de um Autômato com Pilha (AP).
    Usa uma Busca em Largura (BFS) para lidar com o não-determinismo.
    """
    # Uma "configuração" é (Estado, Entrada_Restante, Pilha_Atual)
    # Usamos list(palavra) para a entrada e uma lista para a pilha
    # "Z" é o símbolo inicial padrão da pilha
    configuracao_inicial = (estado_inicial, list(palavra), ["Z"]) 
    
    # A fila armazena todas as "possibilidades" (ramos) da computação
    fila_de_configuracoes = [configuracao_inicial]
    
    # Conjunto para evitar loops infinitos (ex: transição ε, ε ; ε)
    # Armazena tuplos de (nome_estado, entrada_como_string, pilha_como_string)
    visitados = set()
    
    limite_passos = 2000 # Segurança contra loops muito longos
    passos = 0

    while fila_de_configuracoes and passos < limite_passos:
        passos += 1
        
        # Pega a configuração mais antiga da fila (BFS)
        estado_atual, entrada_restante, pilha_atual = fila_de_configuracoes.pop(0)

        # --- DEBUG no Console (opcional) ---
        # print(f"Passo {passos}: E={estado_atual.nome}, In='{''.join(entrada_restante)}', Pilha={pilha_atual}")

        # --- Verificação de Loop ---
        # (Converte a lista da pilha em string para ser 'hashable')
        config_tuple = (estado_atual.nome, "".join(entrada_restante), "".join(pilha_atual))
        if config_tuple in visitados:
            continue # Já explorámos este caminho exato
        visitados.add(config_tuple)

        # --- Condição de Aceitação ---
        # 1. Palavra foi completamente consumida
        # 2. O estado atual é um estado de aceitação
        if not entrada_restante and estado_atual.aceitacao:
            resultado.config(text="Palavra aceita ✅", fg="green")
            # Mostra o estado final da pilha (pode não estar vazia)
            sequencia_saida.config(text="Pilha final: " + "".join(pilha_atual if pilha_atual else ["ε"]))
            return # Encontrou um caminho de aceitação!

        # --- Explorar Transições ---
        simbolo_lido = entrada_restante[0] if entrada_restante else None
        topo_pilha = pilha_atual[-1] if pilha_atual else None # O que está no topo?

        transicoes_aplicaveis = []
        
        # 1. Encontrar transições que consomem o símbolo de entrada
        if simbolo_lido:
            for t in transicoes:
                # Condições:
                # 1. Origem é o estado atual
                # 2. Símbolo de entrada da transição bate com o símbolo lido
                # 3. Símbolo 'pop' da transição bate com o topo da pilha OU é 'ε' (não importa)
        ...
        # 2. Encontrar transições épsilon (sempre verificadas, não consomem entrada)
        for t in transicoes:
             # Condições:
             # 1. Origem é o estado atual
             # 2. Símbolo de entrada da transição é 'ε'
             # 3. Símbolo 'pop' da transição bate com o topo da pilha OU é 'ε'
             if (t.origem == estado_atual and 
        ...
        # 3. Processar todas as transições aplicáveis (adicionar novos caminhos à fila)
        for t, tipo_movimento in transicoes_aplicaveis:
            
            # Prepara a nova pilha (Pop)
            nova_pilha = list(pilha_atual) # Copia a pilha atual
            if t.simbolo_pop == topo_pilha and topo_pilha is not None:
                nova_pilha = pilha_atual[:-1] # Efetua o Pop
            
            # Prepara a nova pilha (Push)
            if t.string_push != "ε":
                # Empilha a string ao contrário (ex: push 'AX' -> empilha 'X' e depois 'A')
            ...
            if tipo_movimento == "consumir_input":
                nova_entrada = entrada_restante[1:] # Consome o símbolo
                
            # Adiciona a nova configuração (o novo "ramo") ao fim da fila
            fila_de_configuracoes.append((t.destino, nova_entrada, nova_pilha))
            
    # --- Fim do loop ---
    # Se a fila esvaziou e não encontrou aceitação, a palavra é rejeitada.
...
def simular_passo_a_passo_Moore(palavra_restante, estado_atual, saida_acumulada, historico_passos):
    # (Esta função permanece inalterada, apenas uma pequena correção na limpeza de realce)
    for est in estados.values(): canvas.itemconfig(est.id_circulo, outline="black", width=2)
    canvas.itemconfig(estado_atual.id_circulo, outline="blue", width=3)
    def limpar_realces_finais():
        for est in estados.values():
    ...
    if not palavra_restante:
        ...
        sequencia_saida.config(text=f"Saída Final: {saida_acumulada}")
        janela.after(500, lambda: salvar_saida_em_arquivo(input_entry.get(), historico_passos))
        janela.after(2000, limpar_realces_finais) # <-- CORRIGIDO
        return
        ...
        "transicoes": [
            {
                ...
                "simbolos_entrada": t.simbolos_entrada,
                "simbolo_saida": t.simbolo_saida,
                "simbolo_pop": t.simbolo_pop,      # <-- NOVO
                "string_push": t.string_push,    # <-- NOVO
                "offset_x": t.offset_x, 
                "offset_y": t.offset_y
            } 
            for t in transicoes
...
def salvar():
    # --- MODIFICADO: Delega a exportação XML para a função salvar_como ---
...
def salvar_como():
    ...
    # --- MODIFICADO: Lógica de decisão movida para cá ---
    if caminho_arquivo_atual.lower().endswith('.jff'):
        exportar_para_jflap_xml(caminho_arquivo_atual)
    else: 
        _salvar_dados_no_arquivo(caminho_arquivo_atual)
...
# --- MODIFICADO: Exporta AP para JFLAP (tags <pop> e <push>) ---
def exportar_para_jflap_xml(caminho_arquivo):
    ...
    # --- LÓGICA MODIFICADA ---
    # Define o tipo correto de autômato no XML
    if tipo_automato_atual == "AP":
        ET.SubElement(raiz, 'type').text = "pda" # 'pda' para Autômato com Pilha
    else:
        ET.SubElement(raiz, 'type').text = "fa" # 'fa' para Autômato Finito
    # --- FIM DA MODIFICAÇÃO ---
    ...
    for transicao in transicoes:
        ...
        # Símbolo de entrada (JFLAP chama de <read>)
        ...
        if tipo_automato_atual == "AP":
            ...
            if simbolo_pop.lower() != "ε":
                pop_tag.text = simbolo_pop
                
            # String Push
            string_push = transicao.string_push
            push_tag = ET.SubElement(t_xml, 'push')
            if string_push.lower() != "ε":
                push_tag.text = string_push
        # --- FIM DAS NOVAS TAGS ---
...
def abrir_automato():
    ...
    # (A função 'importar_de_jflap_xml' agora atualiza o 'tipo_automato_atual' e o 'status_modo')
    corrigir_desvios_carregados()
    # status_acao.config(text=f"Autômato carregado de {arquivo.split('/')[-1]}") # Removido para evitar sobrepor a msg


# --- MODIFICADO: Carrega os novos dados do AP do JSON ---
def _carregar_dados_json(arquivo):
    ...
    try:
        ...
        for e_data in dados["estados"]:
            ...
            # Carrega saída de Moore (se existir)
            estado.simbolo_saida = e_data.get("simbolo_saida", "")
            estado.atualizar_texto() # Atualiza o display (ex: q0/s0)
        ...
        if estados:
            ...
            if numeros_estado:
                ...
                contador_estados = 1 # Inicia em 1 se não houver 'q's
        ...
        for t_data in dados["transicoes"]:
            ...
            transicoes.append(Transicao(
                ...
                t_data.get("simbolo_pop", "ε"),      # <-- NOVO: Carrega pop
                t_data.get("string_push", "ε"),     # <-- NOVO: Carrega push
        ...
        novo_automato() # Reseta o canvas se falhar


# --- MODIFICADO: Importa AP do JFLAP (tags <pop> e <push>) ---
def importar_de_jflap_xml(caminho_arquivo):
    ...
    try:
        ...
        # --- NOVO: Define o tipo de autômato com base no ficheiro ---
        tipo_xml = raiz.find('type')
        if tipo_xml is not None and tipo_xml.text == "pda":
            tipo_automato_atual = "AP"
            print("Ficheiro JFLAP tipo 'pda' detectado. Mudando para modo Autômato com Pilha.")
        else:
            tipo_automato_atual = "AFNe" # Padrão seguro para 'fa'
        
        # Atualiza a UI para refletir o tipo carregado
        atualizar_status_modo() 
        # --- FIM DA MODIFICAÇÃO ---
        ...
        # --- LÓGICA DE IMPORTAR TRANSIÇÕES MODIFICADA ---
        for t_xml in raiz.findall('./automaton/transition'):
            ...
            transicoes.append(Transicao(
                ...
                destino, 
                canvas, 
                simbolos_entrada=simbolo_input,
                simbolo_pop=simbolo_pop,      # <-- NOVO
                string_push=string_push     # <-- NOVO
            ))
        # --- FIM DA MODIFICAÇÃO ---
        ...
        novo_automato() # Reseta se falhar
        return False
...
def corrigir_desvios_carregados():
    ...
    for t1 in transicoes:
        for t2 in transicoes:
            ...
            # Se são transições opostas E ambas estão sem desvio
            if t1.origem == t2.destino and t1.destino == t2.origem and t1.offset_x == 0 and t2.offset_x == 0:
...
# --- UI Setup ---
janela = tk.Tk()
janela.title("Mini-JFLAP em Python v12 (com Autômato com Pilha)") # <-- Título atualizado
...
# --- MODIFICADO: Menu Tipo de Autômato (AP Habilitado) ---
...
try:
    ...
    for nome in nomes_icones:
        # Tenta carregar os ícones (necessita de uma pasta 'icones' no mesmo diretório)
